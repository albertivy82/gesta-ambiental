...
export const useNovoServicoComunicacao = (benfeitoria: BenfeitoriaType, servicoComunicacao?: ServicosComunicacaoType) => {
            ...
            const isConnected = await testConnection();
                  ...
                  if(isConnected){
                    ...
                    try{
                      ...
                      const response = await connectionAPIPost('https://dadoseconomicos.ideflorbio.pa.gov.br/servico-de-comunicacao', novoServicoComunicacao) as ServicosComunicacaoType;
    ...
    const isConnected = await testConnection();
     ...
     if(isConnected){
            ...
            try{
              ...
              const response = await connectionAPIPut(`https://dadoseconomicos.ideflorbio.pa.gov.br/servico-de-comunicacao/${servicoComunicacao!.id}`, servicoComunicacaoCorrigida) as ServicosComunicacaoType;
                    if (response && response.id) {
                      return fetchServicoComunicacaoAPI(response.id);
                    }else{
                      const local = await salvarservicosComunicacao(buildServicoComunicacaoAtualizada());
  ...
  const buildServicoComunicacaoAtualizada = (): ServicosComunicacaoType => ({
   ...
   const fetchServicoComunicacaoAPI = async(id:number) =>{
          ...
          try{
              const response = await connectionAPIGet<ServicosComunicacaoType>(`https://dadoseconomicos.ideflorbio.pa.gov.br/servico-de-comunicacao/${id}`);
              if (response) {
                const servComData = {
                    ...response,
                    sincronizado: true,
