            ...
            const isConnected = await testConnection();
                  ...
                  if(isConnected){
                    ...
                    try{
                      ...
                      const response = await connectionAPIPost('/api/servico-de-comunicacao', novoServicoComunicacao) as ServicosComunicacaoType;
    ...
    const isConnected = await testConnection();
     ...
     if(isConnected){
            ...
            try{
              ...
              const response = await connectionAPIPut(`/api/servico-de-comunicacao/${servicoComunicacao!.id}`, servicoComunicacaoCorrigida) as ServicosComunicacaoType;
                    if (response && response.id) {
                      return fetchServicoComunicacaoAPI(response.id);
                    }else{
                      const local = await salvarservicosComunicacao(buildServicoComunicacaoAtualizada());
