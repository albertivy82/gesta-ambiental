 ...
 export const useBenfeitorias = (imovelId:number, foccus:boolean)=>{
   ...
   const sinconizeBenfeitoriaQueue = async () => {
                    ...
                    if (isConnected) {
                       // console.log("useBenfeitorias/isConnected", isConnected)
                        try {
                            ...
                            const response = await connectionAPIPost('https://dadoseconomicos.ideflorbio.pa.gov.br/benfeitoria', novaBenfeitoriaInput);
    ...
    if (isConnected) {
        ...
        try{
            const response = await connectionAPIGet<BenfeitoriaType[]>(`https://dadoseconomicos.ideflorbio.pa.gov.br/benfeitoria/imovel-benfeitoria/${imovelId}`);
