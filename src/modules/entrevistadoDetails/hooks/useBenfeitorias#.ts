  ...
  imovelId?: number
   ...
   const sinconizeBenfeitoriaQueue = async () => {
                    ...
                    if (isConnected) {
                       // console.log("useBenfeitorias/isConnected", isConnected)
                        try {
                            ...
                            const response = await connectionAPIPost('/api/benfeitoria', novaBenfeitoriaInput);
    ...
    if (isConnected) {
        ...
        try{
            const response = await connectionAPIGet<BenfeitoriaType[]>(`/api/benfeitoria/imovel-benfeitoria/${imovelId}`);
