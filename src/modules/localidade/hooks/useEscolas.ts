...
export const useEscolas= (localidadeId:number, foccus:boolean)=>{
        ...
        if (escolasQueue.length > 0) {
            for (const escola of escolasQueue) {
                ...
                //console.log(novaEscolaIput);
                    ...
                    if (isConnected) {
                        try {
                            const response = await connectionAPIPost('/api/escola', novaEscolaIput);
   ...
   const fetchEscolasromAPI = async () => {
       ...
       const isConnected = await testConnection();
        if (isConnected) {
          try {
              const escolasAPI = await connectionAPIGet<EscolaType[]>(`/api/escola/localidade-escola/${localidadeId}`);
