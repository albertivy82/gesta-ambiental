          ...
          const isConnected = await testConnection();
                ...
                if(isConnected){
                  ...
                  try{
                    ...
                    const response = await connectionAPIPost('https://dadoseconomicos.ideflorbio.pa.gov.br/benfeitoria', novaBenfeitoria) as BenfeitoriaType;
  ...
  const isConnected = await testConnection();
   if(isConnected){
          ...
          try{
            ...
            const response = await connectionAPIPut(`https://dadoseconomicos.ideflorbio.pa.gov.br/benfeitoria/${benfeitoria!.id}`, benfeitoriaCorrigida) as BenfeitoriaType;
                if (response && response.id) {
                return fetchBefeitoriaAPI(response.id);
                }else{
                  const local = await salvarBenfeitoria(buildBenfeitoriaAtualizada());
...
const buildBenfeitoriaAtualizada = (): BenfeitoriaType => ({
 ...
 const fetchBefeitoriaAPI = async(id:number) =>{
        ...
        try{
            const response = await connectionAPIGet<BenfeitoriaType>(`https://dadoseconomicos.ideflorbio.pa.gov.br/benfeitoria/${id}`);
            if (response) {
              const bftData = {
                  ...response,
                  sincronizado: true,
