          ...
          const isConnected = await testConnection();
                ...
                if(isConnected){
                  ...
                  try{
                    ...
                    const response = await connectionAPIPost('/api/benfeitoria', novaBenfeitoria) as BenfeitoriaType;
  ...
  const isConnected = await testConnection();
   if(isConnected){
          ...
          try{
            ...
            const response = await connectionAPIPut(`/api/benfeitoria/${benfeitoria!.id}`, benfeitoriaCorrigida) as BenfeitoriaType;
                if (response && response.id) {
                return fetchBefeitoriaAPI(response.id);
                }else{
                  const local = await salvarBenfeitoria(buildBenfeitoriaAtualizada());
...
const buildBenfeitoriaAtualizada = (): BenfeitoriaType => ({
 ...
 const fetchBefeitoriaAPI = async(id:number) =>{
        ...
        try{
            const response = await connectionAPIGet<BenfeitoriaType>(`/api/benfeitoria/${id}`);
            if (response) {
              const bftData = {
                  ...response,
                  sincronizado: true,
