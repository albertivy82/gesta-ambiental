    ...
    const [disabled, setDisabled] = useState<boolean>(true);
     ...
     useEffect(()=>{
        ...
        const registrarCoordenada = async (localidadeId: number) => {
            ...
            const resultBack = await connectionAPIPost(`https://dadoseconomicos.ideflorbio.pa.gov.br/coordenada`, coordenadaNova);
        ...
        const atualizarCoordenada = async (coordenadaId: number) => {
           ...
           const resultBack = await connectionAPIPut(`https://dadoseconomicos.ideflorbio.pa.gov.br/coordenada/${coordenadaId}`, coordenadaNova);
